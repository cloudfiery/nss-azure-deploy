...
if ( $confirm -eq 'y' )
{
    ...
    $Command2 = 'sed -i -e '+"'2s/^/MY_IP=$MY_IP ; NEW_NAME_SERVER_IP=$NEW_NAME_SERVER_IP /'"+' script.sh'
    ...
    if ($confirm2 -eq 'y')
    {
        ...
        $Command2 = 'sed -i -e '+"'2s/^/MY_IP=$MY_IP ; NEW_NAME_SERVER_IP=$NEW_NAME_SERVER_IP ; NEW_NAME_SERVER_IP2=$NEW_NAME_SERVER_IP2 /'"+' script.sh'
    }    
}
else 
{
